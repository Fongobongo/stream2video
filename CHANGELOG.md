...
- **Broken-PTS fallback** — if the D-sample and A-sample disagree (e.g., sources with `itsoffset` or corrupted timestamps), the freshly-extracted WAV is deleted and a full A detection is run. Logs a WARNING naming the segment count mismatch.
