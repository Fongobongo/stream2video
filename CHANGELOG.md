...
- **Batch pipeline works on ffmpeg 9.x** — the per-chunk encode fed its filter graph through `-filter_complex_script`, a flag the 9.x builds (now shipped by choco/apts CI installs) removed (`Unrecognized option 'filter_complex_script'` → every batch run failed). The pipeline now probes the ffmpeg version once and picks `-filter_complex_script` on ffmpeg < 7 or the equivalent `-/filter_complex <file>` on ≥ 7 (where that syntax landed); both keep the graph in a file so long keep-segment lists stay under Windows's 32K command-line limit.
