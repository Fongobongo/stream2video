...
- [x] Сравнить ultrafast/veryfast/fast/medium — ✅ выполнено (standalone benchmark script `scripts/benchmark_presets.py` — генерирует синтетический source через lavfi `testsrc+sine`, прогоняет `cut_and_concat` с каждым `x264_preset` из `--presets` (default `ultrafast,veryfast,fast,medium`; валидируются против `VALID_X264_PRESETS`), замеряет wall-clock time / output MiB / video bitrate (ffprobe), печатает comparative таблицу (preset / time_s / size_mb / bitrate_kbps / speed / ratio vs baseline). Default `--repeat 3` с median-of-N для устойчивости, `--method segment|batch|cut_then_encode`, `--duration 10 --fps 30 --size 320x240`, `--video-quality`, `--work-dir`/`--keep-source`. Sample run на 10s source: ultrafast 1.06s/0.23 MiB/142 kbps, veryfast 1.09s/0.16 MiB/47 kbps, fast 1.53s/0.17 MiB/52 kbps, medium 1.29s/0.16 MiB/47 kbps — типичный trade-off ultrafast (быстро, большой файл) vs veryfast/medium (сходная скорость, 71% размер).)
...
- [x] `prlimit` / `RLIMIT_AS` — ✅ выполнено (`rlimit_as_mb` config key / CLI `--rlimit-as-mb N` (default 0 = off, POSIX-only). `subprocess_kwargs(low_priority, rlimit_as_mb)` в utils.py — compose в один `preexec_fn` (Python только позволяет один) — `os.nice(10)` + `resource.setrlimit(RLIMIT_AS, (cap, cap))`. `cap = rlimit_as_mb * 1024 * 1024`. На Windows — no-op (нет portable RLIMIT_AS), `memory_limit_mb` pre-flight остается единственной memory door. Проброшен через concat.py параллельно с `low_process_priority` — `cut_and_concat → _run_with_fallback → _try → _run_segment_concat / _run_cut_then_encode / _run_batch_concat → _run_ffmpeg / _run_final_concat / _run_audio_extract / _run_audio_concat_filter / _run_gapless_segment_concat`. 9 unit-тестов `TestSubprocessKwargs` для rlimit: returns_only_no_window (rc=0 default), ignored_on_windows, + low_priority compose в один preexec_fn, rlimit-only posix, real POSIX kernel-enforced cap (запуск python -c "bytearray(4 MiB)" с rlimit_as_mb=2 → MemoryError → rc=0; high cap sanity (512 MiB allows python interpreter startup). 1 media correctness (`test_rlimit_as_mb_smoke` rl=1024 на 6s synthetic source, 90 frames at 30fps). Дополнение к in-process `memory_limit_mb` pre-flight: он сэмплирует RSS *между* wall-clock polls и может пропустить быстрый spike — RLIMIT_AS блокирует на kernel уровне до того, как malloc вообще вернёт память.)
